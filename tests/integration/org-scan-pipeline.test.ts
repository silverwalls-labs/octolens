...
describe('org scan pipeline', () => {
	...
	test('full org scan with all rules produces valid results', async () => {
		mockCompliantOrg();
	...
	test('non-compliant org flags every rule', async () => {
		mockNonCompliantOrg();
		 ...
		 * org/actions-allowlist-pinned is not applicable.
		 */
		assert.equal(result.summary.rulesRun, allOrgRules.length);
		assert.equal(
			result.summary.findingsTotal,
			allOrgRules.length - 1,
		);
	...
	test(
		'non-admin token skips every rule instead of passing',
		async () => {
			mockNonAdminOrg();
	...
	test(
		'rate-limit 403 on an org endpoint errors instead of skipping',
		async () => {
			nock(BASE).get(`/orgs/${ORG}`)
				.reply(200, makeOrgResponse());
			nock(BASE).persist()
				.get(/\/orgs\/silverwalls-labs\/actions/)
				.reply(
					403,
	...
	test(
		'500 from all endpoints produces errored rules, not a crash',
		async () => {
	...
	test(
		'markdown output renders the org header and findings',
		async () => {
			mockNonCompliantOrg();
			...
			assert.match(
				md,
				/`org\/two-factor-required` · silverwalls-labs/,
			);
	...
	test(
		'pretty output renders the org header and findings',
		async () => {
			mockNonCompliantOrg();
			...
			assert.match(
				pretty,
				/Octolens scan — silverwalls-labs/,
			);
	...
	test(
		'JSON output round-trips and findings carry org, not repo',
		async () => {
			mockNonCompliantOrg();
...
function mockNonAdminOrg(): void {
	...
	nock(BASE).persist()
		.get(/\/orgs\/silverwalls-labs\/(actions|hooks)/)
