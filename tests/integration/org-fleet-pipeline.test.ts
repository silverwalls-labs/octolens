...
describe('org fleet pipeline', () => {
	...
	test(
		'fleet scan covers the org and every eligible repository',
		async () => {
			mockCompliantOrg();
			mockListing([
				{ name: 'app' },
	...
	test(
		'a persistently failing repo never aborts the fleet scan',
		async () => {
			mockCompliantOrg();
			...
			assert.equal(
				exitCodeForReport(report, { failOnIncomplete: true }),
				1,
			);
		},
	);

	test('all three formatters render the fleet report', async () => {
		mockCompliantOrg();
		...
		assert.match(
			pretty,
			/Octolens scan — silverwalls-labs \(organization \+ 1 repositories\)/,
		);
		assert.match(pretty, /Repositories: 1 scanned/);
	});

	test(
		'a failing listing truncates the fleet scan but still reports',
		async () => {
			mockCompliantOrg();
	...
	test(
		'a depleted rate budget pauses intake before the next repository',
		async () => {
			mockCompliantOrg();
			nock(BASE).get(`/orgs/${ORG}/repos`)
				.query({ per_page: '100', type: 'all' })
			...
			assert.match(
				stderrText,
				/rate budget low \(10 remaining/,
			);
			assert.match(
				stderrText,
				/rate budget window reset; resuming/,
			);
...
function mockScannableRepo(name: string): void {
	nock(BASE).get(`/repos/${ORG}/${name}`)
		.reply(200, makeRepoResponse());
	nock(BASE).persist()
		.get(new RegExp(`/repos/${ORG}/${name}/`))
		.reply(200, []);
	nock(BASE).persist()
		.head(new RegExp(`/repos/${ORG}/${name}/`))
		.reply(204);
}
