	...
	references: [
		{
			...
			url: 'https://docs.github.com/protected',
		},
	],
};
...
describe('formatMarkdown', () => {
	test(
		'renders header, summary table, and finding sections',
		() => {
			const md = formatMarkdown(makeResult([ SAMPLE ]));

			assert.match(
				md,
				/^# Octolens scan — sheplu\/editorconfig/,
			);
			...
			assert.match(
				md,
				/### Critical — Default branch 'main' is not protected/,
			);
			assert.match(
				md,
				/`repo-config\/branch-protection-required` · sheplu\/editorconfig/,
			);
			assert.match(
				md,
				/\*\*Remediation:\*\* Add a branch protection rule\./,
			...
			assert.match(
				md,
				/- \[About protected branches\]\(https:\/\/docs\.github\.com\/protected\)/,
			);
		},
	);

	test(
		'renders a clean-scan message when there are no findings',
		() => {
			const md = formatMarkdown(makeResult([]));

			assert.match(
				md,
				/No findings at or above the `info` threshold\./,
			);
			assert.doesNotMatch(md, /## Findings/);
		},
	);

	test('orders findings by severity descending', () => {
	...
	test(
		'renders a checks table covering every rule run',
		() => {
			const runs: RuleRun[] = [
				{
					ruleId: 'repo-config/branch-protection-required',
			...
			assert.match(
				md,
				/flagged \| `repo-config\/branch-protection-required` \| 1 \|/,
			);
			assert.match(
				md,
				/pass \| `repo-config\/license-file-present` \| — \|/,
			);
			assert.match(
				md,
				/skipped \| `access\/required-custom-properties` \| — \|/,
			);
			assert.match(
				md,
				/error \| `access\/outside-collaborator-count` \| — \|/,
			);
		},
	);

	test(
		'omits the checks section when no runs are present',
		() => {
			const md = formatMarkdown(makeResult([ SAMPLE ]));
	...
	test('renders org target in header', () => {
