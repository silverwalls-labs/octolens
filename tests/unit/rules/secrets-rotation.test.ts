...
describe('security/secrets-rotation', () => {
	...
	test('reports no findings when no secrets exist', async () => {
		mockEmpty();
	...
	test('reports no findings when all secrets are within rotation window', async () => {
	...
	test('reports a finding aggregating stale secrets across stores', async () => {
	...
	test('treats secrets with unparseable dates as fresh', async () => {
		const badBody = {
			total_count: 1,
			secrets: [
				{
	...
	test('skips when all secret endpoints return a permission 403', async () => {
	...
	test('unreadable stores are ignored when another store is readable', async () => {
		mockSecrets(ACTIONS, 403);
