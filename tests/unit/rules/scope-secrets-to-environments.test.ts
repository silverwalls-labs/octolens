...
describe('security/scope-secrets-to-environments', () => {
	beforeEach(disableNet);
	afterEach(restoreNet);

	test('reports no findings when repo has no Actions secrets', async () => {
	...
	test('reports a finding when repo-level Actions secrets exist', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.query({ per_page: '100' })
			.reply(200, {
	...
	test('skips when the secrets endpoint returns a permission 403', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.query({ per_page: '100' })
