...
describe('repo-config/auto-delete-head-branches', () => {
	beforeEach(disableNet);
	afterEach(restoreNet);

	test('reports no findings when delete_branch_on_merge is true', async () => {
	...
	test('reports a finding when delete_branch_on_merge is false', async () => {
		nock('https://api.github.com')
			.get('/repos/sheplu/Octolens')
	...
	test('reports a finding when the field is missing', async () => {
		nock('https://api.github.com')
			.get('/repos/sheplu/Octolens')
			.reply(200, {
				name: 'Octolens',
