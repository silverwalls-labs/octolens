...
describe('repo-config/require-linear-history', () => {
	beforeEach(disableNet);
	afterEach(restoreNet);

	test('reports no findings when linear history is required', async () => {
	...
	test('reports a finding when linear history is not required', async () => {
		nock('https://api.github.com')
			.get('/repos/sheplu/Octolens')
			.reply(200, makeRepoResponse());
		nock('https://api.github.com')
