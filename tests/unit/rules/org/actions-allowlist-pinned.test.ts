...
test('reports no findings when every pattern is SHA-pinned', pinnedCase);

async function pinnedCase() {
...
test('reports no findings when the allowlist is not in use', notSelectedCase);

async function notSelectedCase() {
	nock('https://api.github.com')
		.get(PERMISSIONS)
...
test('reports a finding listing wildcard and tag patterns', unpinnedCase);

async function unpinnedCase() {
	nock('https://api.github.com')
		.get(PERMISSIONS)
		.reply(200, makeOrgActionsPermissionsResponse({ allowedActions: 'selected' }));
	nock('https://api.github.com')
...
test('skips when the allowlist itself is not readable', selectedForbiddenCase);

async function selectedForbiddenCase() {
	nock('https://api.github.com')
		.get(PERMISSIONS)
		.reply(200, makeOrgActionsPermissionsResponse({ allowedActions: 'selected' }));
	nock('https://api.github.com')
