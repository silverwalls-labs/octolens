...
import {
	rule,
} from '../../../../src/rules/org/secret-scanning-push-protection-for-new-repos.ts';
...
describe('org/secret-scanning-push-protection-for-new-repos', () => {
	beforeEach(disableNet);
	afterEach(restoreNet);

	test('reports no findings when push protection is enabled for new repos', async () => {
	...
	test('reports a finding when push protection is not enabled for new repos', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.reply(200, makeOrgResponse({
		...
		assert.equal(findings[0]?.ruleId, 'org/secret-scanning-push-protection-for-new-repos');
