...
import {
	rule,
} from '../../../../src/rules/org/members-cannot-invite-outside-collaborators.ts';
...
describe('org/members-cannot-invite-outside-collaborators', () => {
	beforeEach(disableNet);
	afterEach(restoreNet);

	test('reports no findings when members cannot invite outside collaborators', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.reply(200, makeOrgResponse({ membersCanInviteOutsideCollaborators: false }));
	...
	test('reports a finding when members can invite outside collaborators', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.reply(200, makeOrgResponse({ membersCanInviteOutsideCollaborators: true }));

		const findings = await rule.check(makeOrgContext());

		assert.equal(findings.length, 1);
		assert.equal(findings[0]?.ruleId, 'org/members-cannot-invite-outside-collaborators');
