...
describe('org/secret-scanning-for-new-repos', () => {
	beforeEach(disableNet);
	afterEach(restoreNet);

	test('reports no findings when secret scanning is enabled for new repos', async () => {
	...
	test('reports a finding when secret scanning is not enabled for new repos', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
