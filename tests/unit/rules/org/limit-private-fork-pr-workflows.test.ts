...
test('reports no findings when fork PR workflows are disabled', disabledCase);
...
test('reports no findings when enabled but fully hardened', hardenedCase);

async function hardenedCase() {
	nock('https://api.github.com')
		.get(ENDPOINT)
		.reply(200, makeOrgPrivateForkPrResponse({
...
test('reports a finding listing each weak setting', weakCase);

async function weakCase() {
	nock('https://api.github.com')
		.get(ENDPOINT)
		.reply(200, makeOrgPrivateForkPrResponse({
			runWorkflowsFromForkPullRequests: true,
