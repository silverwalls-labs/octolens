...
import { rule } from '../../../../src/rules/org/members-cannot-delete-issues.ts';
...
describe('org/members-cannot-delete-issues', () => {
	beforeEach(disableNet);
	afterEach(restoreNet);

	test('reports no findings when members cannot delete issues', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.reply(200, makeOrgResponse({ membersCanDeleteIssues: false }));
	...
	test('reports a finding when members can delete issues', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.reply(200, makeOrgResponse({ membersCanDeleteIssues: true }));

		const findings = await rule.check(makeOrgContext());

		assert.equal(findings.length, 1);
		assert.equal(findings[0]?.ruleId, 'org/members-cannot-delete-issues');
