...
import { rule } from '../../../../src/rules/org/members-cannot-create-public-repos.ts';
...
describe('org/members-cannot-create-public-repos', () => {
	beforeEach(disableNet);
	afterEach(restoreNet);

	test('reports no findings when members cannot create public repos', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.reply(200, makeOrgResponse({ membersCanCreatePublicRepositories: false }));
	...
	test('reports a finding when members can create public repos', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.reply(200, makeOrgResponse({ membersCanCreatePublicRepositories: true }));

		const findings = await rule.check(makeOrgContext());

		assert.equal(findings.length, 1);
		assert.equal(findings[0]?.ruleId, 'org/members-cannot-create-public-repos');
