...
describe('access/deploy-keys-readonly', () => {
	beforeEach(disableNet);
	afterEach(restoreNet);

	test('reports no findings when no deploy keys exist', async () => {
	...
	test('reports no findings when all deploy keys are read-only', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.query({ per_page: '100' })
			.reply(200, [
	...
	test('reports a finding when deploy keys allow writes', async () => {
		nock('https://api.github.com')
			.get(ENDPOINT)
			.query({ per_page: '100' })
			.reply(200, [
