...
describe('queries edge cases', () => {
	...
	test(
		'custom property values return null on 404',
		async () => {
			nock(BASE)
				.get('/repos/sheplu/Octolens/properties/values')
				.reply(404);

			const values = await getRepoCustomPropertyValues(
				makeOctokit(),
				createCachedFetcher(),
				REPO,
			);
	...
	test(
		'custom property values return null on permission-denied 403',
		async () => {
			nock(BASE)
				.get('/repos/sheplu/Octolens/properties/values')
				.reply(403, {
	...
	test(
		'custom property values propagate server errors',
		async () => {
			nock(BASE)
				.get('/repos/sheplu/Octolens/properties/values')
				.reply(500);

			await assert.rejects(getRepoCustomPropertyValues(
				makeOctokit(),
				createCachedFetcher(),
				REPO,
			));
	...
	test(
		'custom property values propagate rate-limited 403s',
		async () => {
			nock(BASE)
				.get('/repos/sheplu/Octolens/properties/values')
				.reply(
	...
	test(
		'collaborator permissions fall back to the legacy permissions object',
		async () => {
			nock(BASE)
			...
			const collaborators = await getDirectCollaborators(
				makeOctokit(),
				createCachedFetcher(),
				REPO,
			);
			...
			assert.deepEqual(
				collaborators.map(toPermission),
				[
					'admin',
					'maintain',
					'write',
	...
	test(
		'org metadata tolerates a minimal payload',
		async () => {
			nock(BASE)
				.get('/orgs/silverwalls-labs')
				.reply(200, { login: 'silverwalls-labs' });
			...
			assert.equal(
				meta.twoFactorRequirementEnabled,
				undefined,
			);
			assert.equal(
				meta.defaultRepositoryPermission,
				undefined,
			);
			assert.equal(
				meta.membersCanCreatePublicRepositories,
				undefined,
			);
		},
	);
	...
	test(
		'webhook insecure_ssl accepts numeric values',
		async () => {
			nock(BASE)
	...
	test(
		'branch protection propagates server errors',
		async () => {
			nock(BASE)
				.get('/repos/sheplu/Octolens/branches/main/protection')
				.reply(500);

			await assert.rejects(getBranchProtection(
				makeOctokit(),
				createCachedFetcher(),
				REPO,
				'main',
	...
	test(
		'vulnerability reporting propagates server errors',
		async () => {
			nock(BASE)
				.get('/repos/sheplu/Octolens/private-vulnerability-reporting')
				.reply(500);

			await assert.rejects(getPrivateVulnerabilityReporting(
				makeOctokit(),
				createCachedFetcher(),
				REPO,
			));
	...
	test(
		'runner listing propagates server errors',
		async () => {
			nock(BASE)
				.get('/repos/sheplu/Octolens/actions/runners')
				.query(true)
				.reply(500);
	...
	test(
		'webhook listing propagates server errors',
		async () => {
			nock(BASE)
				.get('/repos/sheplu/Octolens/hooks')
				.query(true)
	...
	test(
		'deploy key listing propagates server errors',
		async () => {
			nock(BASE)
				.get('/repos/sheplu/Octolens/keys')
				.query(true)
				.reply(500);
	...
	test('webhook listing returns empty on 404', async () => {
		nock(BASE)
			.get('/repos/sheplu/Octolens/hooks')
			.query(true)
			.reply(404);
	...
	test(
		'deploy key listing returns empty on 404',
		async () => {
			nock(BASE)
				.get('/repos/sheplu/Octolens/keys')
				.query(true)
	...
	test(
		'org allowed-actions propagate server errors',
		async () => {
			nock(BASE)
				.get('/orgs/silverwalls-labs/actions/permissions/selected-actions')
				.reply(500);

			await assert.rejects(getOrgAllowedActions(
				makeOctokit(),
				createCachedFetcher(),
				'silverwalls-labs',
			));
	...
	test(
		'isRateLimited classifies rate-limit signals',
		() => {
			assert.equal(
			...
			assert.equal(
				isRateLimited({ status: 500 }),
				false,
			);
			assert.equal(
				isRateLimited('not an object'),
				false,
			);
