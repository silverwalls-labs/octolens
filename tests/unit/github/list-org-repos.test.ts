...
import {
	listOrgRepos, type OrgRepoListing,
...
describe('listOrgRepos', () => {
	...
	test(
		'streams and maps a single page of repositories',
		async () => {
	...
	test('follows pagination across pages', async () => {
		const nextLink =
			`<${BASE}/orgs/${ORG}/repos?` +
			'per_page=100&type=all&page=2>; rel="next"';
	...
	test(
		'propagates listing errors to the caller',
		async () => {
			nock(BASE).get(`/orgs/${ORG}/repos`)
				.query({ per_page: '100', type: 'all' })
