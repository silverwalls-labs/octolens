...
describe('scanOrg', () => {
	...
	test(
		'scanOrg rolls up findings and computes summary',
		async () => {
	...
	test(
		'scanOrg filters out findings below threshold',
		async () => {
			nock('https://api.github.com')
				.get('/orgs/silverwalls-labs')
				.reply(200, makeOrgResponse({
	...
	test('disabled org rules in config are skipped', async () => {
		const result = await scanOrg({
			org: 'silverwalls-labs',
			rules: [ twoFactorRequired ],
			octokit: makeOctokit(),
			...
			threshold: 'high',
			config: {
				rules: { 'org/two-factor-required': 'off' },
			},
		});
	...
	test(
		'non-owner tokens record skipped runs, not passes',
		async () => {
			nock('https://api.github.com')
				.get('/orgs/silverwalls-labs')
				.reply(200, makeOrgResponse({
	...
	test(
		'org rule errors are captured without aborting the scan',
		async () => {
			nock('https://api.github.com')
				.get('/orgs/silverwalls-labs')
